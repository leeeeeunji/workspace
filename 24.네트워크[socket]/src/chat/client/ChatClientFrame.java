...
public class ChatClientFrame extends JFrame {
	...
	public ChatClientFrame() throws Exception {
		...
		chatList.setModel(new AbstractListModel() {
			String[] values = new String[] {"승연", "규리", "니콜", "지영", "영지", "하라"};
			public int getSize() {
	...
	/*****************************************************
	 * 클라이언트쪽 소켓을 사용하여 서버와 통신을 담당하는 클래스(VO) 
	 * - 서버와 연결된 소켓1개를 가지고 있는 클래스 
	...
	public ClientClientThread() throws Exception {
		this.socket = new Socket("192.168.15.10", 8888);
	...
	public void run() {
		try {
		while(true) {
			System.out.println("A. ClientCleintThread : 서버로부터 오는 데이타를 읽기 위해 쓰레드 무한대기");
			...
			if(chatArray[0].equals(ChatProtocol.PLAIN_MSG)) {
				System.out.println("B. ClientCleintThread : 서버로부터 읽은 데이타를 클라이언트 채팅창에 보여준다");
				displayMessage(chatArray[1]);
			}else if(chatArray[0].equals(ChatProtocol.LIST_MSG)) {
				System.out.println("B. ClientCleintThread : 서버로부터 읽은 LIST_MSG 데이타로 클라이언트 LIST 갱신");
