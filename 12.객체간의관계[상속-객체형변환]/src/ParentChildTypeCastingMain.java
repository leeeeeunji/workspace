/*
 객체의 형변환
   1. 부모<->자식 관계에 있는 클래스들에서만 가능하다.
      ex> 자식타입 객체의 주소를 부모타입 객체의 주소로 형변환(주소변경 절대 안 됨,타입만 변경)
      ex> 부모타입 객체의 주소를 자식타입 객체의 주소로 형변환(주소변경 절대 안 됨,타입만 변경)
        
   2. 자식객체타입의 객체 주소를 부모클래스타입 객체의 주소로 형변환
       - 자동으로 이루어진다(묵시적)
       Child c = new Child();
       Parent p = c; 
...
public class ParentChildTypeCastingMain {
	...
	public static void main(String[] args) {
		 ...
		 * - 부모의 탈을 쓴 자식객체는 가능하다.
		 */
		Parent p3 = new Child(); //-> p3 = 부모 탈을 쓴 자식객체
