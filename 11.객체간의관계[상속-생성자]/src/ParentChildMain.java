class Parent  {
	...
	public Parent() {
		 ...
		 */
		super(); // 생략가능, 이 위에는 아무 코드도 없어야 함
		System.out.println("1. Object()기본생성자 호출됨");
...
class Child extends Parent {
	...
	public Child() {
		/*
		 - 클래스 생성자 첫번째 라인에서는 부모클래스의 생성자가 반드시 호출 되어야 합니다.
		 - 부모클래스 생성자호출을 생략할 경우에는 부모클래스의 기본생성자가 자동으로 호출 된다. 
		 - 매개변수를 가진 부모의 생성자를 호출할 수 있다.(명시적으로 호출 코드를 기술하여야 한다.)
		 */
		super();
